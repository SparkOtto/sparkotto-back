...
class AuthController {
    ...
    /**
     * Déblocage d'un compte utilisateur (admin seulement)
     * @param req
     * @param res
     */
    async unlockAccount(req: Request, res: Response): Promise<void> {
        try {
            ...
            const user = await this.userService.unlockUserAccount(parseInt(userId));
    ...
    /**
     * Blocage d'un compte utilisateur (admin seulement)
     * @param req
     * @param res
     */
    async lockAccount(req: Request, res: Response): Promise<void> {
        try {
            ...
            const user = await this.userService.lockUserAccount(parseInt(userId));
            ...
            if (error.message === 'Utilisateur non trouvé') {
                ...
                res.status(500).json({ message: 'Erreur lors du blocage du compte' });
            }
        }
    }
